...
DROP POLICY IF EXISTS "Public can view digital products" ON storage.objects;
DROP POLICY IF EXISTS "Admins can manage all digital products" ON storage.objects;
...
CREATE POLICY "Public can view digital products" ON storage.objects
    FOR SELECT USING (bucket_id = 'digital-products');

CREATE POLICY "Admins can manage all digital products" ON storage.objects
    FOR ALL USING (
        bucket_id = 'digital-products' AND
        EXISTS (
            SELECT 1 FROM user_profiles 
...
-- Now we can safely add the digital_product_id column to cart_items (it should already exist from the previous migration)
